...
(function (RT) {
  ...
  function resourceBarHTML(opts) {
    ...
    return ''
      ...
      + '    </div>'
      + '  </div>'
      + '  <div class="rt-resource" id="rt-res-watchtime" style="display:none">'
      + '    <span class="rt-resource__icon">⏱️</span>'
      + '    <div>'
      + '      <div class="rt-resource__label">Watchtime</div>'
      + '      <div class="rt-resource__value">Ø <span data-rt-res="watchtime">0,3</span> Std./Tag</div>'
      + '    </div>'
      + '  </div>'
      + (withRep
        ? '  <div class="rt-resource rt-resource--rep">'
          ...
          + '        <span class="rt-rep-value" id="rt-rep-value">0,20%</span>'
          + '      </div>'
      ...
      + '        <div class="rt-server-bar__seg rt-server-bar__seg--meta" id="rt-server-seg-meta"></div>'
      + '      </div>'
      + '    </div>'
      + '  </div>'
      + '</div>';
    // Team-Kachel entfernt — Team wird jetzt komplett rechts in der Slot-Sidebar dargestellt.
  }
  ...
  function bindResourceBar(container, opts) {
    ...
    var refs = {
      ...
      serverSegMeta:   container.querySelector('#rt-server-seg-meta'),
      ...
      watchtimeEl:     container.querySelector('[data-rt-res="watchtime"]'),
      watchtimeRes:    container.querySelector('#rt-res-watchtime'),
      resBar:          container.querySelector('.rt-resources')
    };
    ...
    function refresh() {
      ...
      var sWt        = RT.state.get();
      var metaActive = !!(sWt.metadatenActive);
      var wt         = sWt.watchtime || 0.3;
      if (refs.watchtimeEl)  refs.watchtimeEl.textContent  = (Math.round(wt * 10) / 10).toFixed(1).replace('.', ',');
      if (refs.watchtimeRes) refs.watchtimeRes.style.display = metaActive ? '' : 'none';
      if (refs.resBar)       refs.resBar.classList.toggle('rt-resources--with-watchtime', metaActive);
      var swPct, usrPct, metaPct;
      if (metaActive) {
        swPct   = cap > 0 ? Math.min(100, swUsage        / cap * 100) : 0;
        usrPct  = cap > 0 ? Math.min(100, usrUsage / 2   / cap * 100) : 0;
        metaPct = cap > 0 ? Math.min(100, usrUsage / 2   / cap * 100) : 0;
      } else {
        swPct   = cap > 0 ? Math.min(100, swUsage  / cap * 100) : 0;
        usrPct  = cap > 0 ? Math.min(100, usrUsage / cap * 100) : 0;
        metaPct = 0;
      }
      ...
      if (refs.serverSegMeta) refs.serverSegMeta.style.width = metaPct.toFixed(2) + '%';
      ...
      var moneyCard = refs.moneyCanvas ? refs.moneyCanvas.parentElement : null;
      if (moneyCard) moneyCard.classList.toggle('rt-resource--money-negative', money < 0);
    }
    ...
    function refreshRep() {
      ...
      // Balken-Skala: -2 % (links) … 0 % (Marker bei 33,33 %) … +4 % (rechts)
      var t = (rep - (-0.02)) / (0.04 - (-0.02));
    ...
    var unsubWt    = RT.bus.on('watchtime:changed',   refresh);
