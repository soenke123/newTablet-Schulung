...
(function (RT) {
  ...
  function totalCampBonus(users) {
    ...
    for (var i = 0; i < camps.length; i++) {
      ...
      if (eType === 'users_per_month')  total += Math.round(eVal * mult);
      else if (eType === 'users_percent') total += Math.floor(users * eVal * mult);
    }
    return total;
  }
  ...
  function renderContent(inner) {
    ...
    var serverWarnHTML = serverFull
      ? '<div class="rt-ul-server-warning">⚠️ Serverkapazität erreicht – keine neuen User können sich registrieren</div>'
      : '';

    var header = '<div class="rt-finance-header">'
      + '<h2 class="rt-finance-header__title">👥 User-Zuwachs</h2>'
      + '<button class="rt-finance-close" id="rt-ul-close" title="Schlie\xDFen">✕</button>'
      + '</div>';

    if (phase === 'garage') {
      // Garage-Phase: Wachstum kommt aus Wachstumsrate (grün) und einmaligen Node-Boni (blau).
      var rate        = s.userGrowthRate || 0;
      var history     = (s.sparkHistory && s.sparkHistory.users) || [];
      var monthlyGain = (users > 0 && rate > 0) ? Math.max(1, Math.floor(users * rate)) : 0;
      var rateStr     = (rate * 100).toFixed(0) + ' %';
      var totSign     = monthlyGain > 0 ? '+' : '';
      var totCls      = monthlyGain > 0 ? 'rt-finance-pos' : 'rt-ul-none';

      var growthRow;
      if (rate > 0) {
        growthRow = '<div class="rt-finance-row">'
          + '<span>Wachstumsrate (' + rateStr + '/Mon)</span>'
          + '<span class="rt-finance-pos">+' + fmt(monthlyGain) + '</span>'
          + '</div>';
      } else {
        growthRow = '<div class="rt-finance-row">'
          + '<span>Wachstumsrate</span>'
          + '<span class="rt-ul-none">–</span>'
          + '</div>';
      }
      ...
      inner.innerHTML = header
        ...
        +       '<span class="rt-finance-legend__item rt-ul-legend--rep">Wachstumsrate</span>'
        +       '<span class="rt-finance-legend__item rt-ul-legend--camp">Einmalige Boni</span>'
        +     '</div>'
        +   '</div>'
        ...
        +       '<span class="' + totCls + '">' + totSign + fmt(monthlyGain) + '</span>'
        +     '</div>'
        +     serverWarnHTML
        +   '</div>'
        + '</div>';
      ...
      setTimeout(function () {
        ...
        if (canvas) drawChart(canvas, history, rate);
      }, 0);
    } else {
      // Campus / Expansion: Ruf-Mundpropaganda + Werbekampagnen
      var history   = (s.sparkHistory && s.sparkHistory.users) || [];
      ...
      var totSign2 = totalGrowth >= 0 ? '+' : '';
      var repCls   = repDelta    >= 0 ? 'rt-finance-pos' : 'rt-finance-neg';
      var totCls2  = totalGrowth >= 0 ? 'rt-finance-pos' : 'rt-finance-neg';
      ...
      inner.innerHTML = header
        ...
        +     '<div class="rt-finance-row rt-finance-row--profit">'
        +       '<span>Zuwachs / Mon</span>'
        +       '<span class="' + totCls2 + '">' + totSign2 + fmt(totalGrowth) + '</span>'
        +     '</div>'
        +     serverWarnHTML
        +   '</div>'
        + '</div>';
