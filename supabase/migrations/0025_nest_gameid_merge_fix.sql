-- ══════════════════════════════════════════════════════════════
-- Migration 0025 — nests[].gameId/gameUrl Merge-Bug + Cleanup
-- ══════════════════════════════════════════════════════════════
-- Analog zu Migration 0024 (atariNumber): shop_state_merge nutzte
-- für nests[].gameId und nests[].gameUrl `coalesce(sv->'X', cv->'X')`.
-- Beim Nest-Anlegen im Shop schreibt der Client jedoch initial
-- `gameId: null, gameUrl: null` (JSON null) — der erste Sync
-- persistiert das auf dem Server. Ab dann kann der Client die
-- Verknüpfung nicht mehr durchsetzen, weil `coalesce(jsonb 'null',
-- jsonb 'game9')` bleibt bei `jsonb 'null'` (JSON null ist für
-- coalesce nicht SQL NULL).
--
-- Symptom: User verknüpft Legi-Nest mit einem Spiel und spielt es;
-- beim Rückkehr in den Hub steht wieder "Spiel noch nicht gewählt"
-- (weil beim Sync gameId auf null geplättet wird).
--
-- Fix:
--   1. shop_state_merge: nullif(..., 'null'::jsonb) für gameId/gameUrl.
--   2. Cleanup: bestehende JSON-null-Werte in nests[].gameId und
--      nests[].gameUrl entfernen (Keys raus), damit beim nächsten
--      Sync der Client-Wert sauber übernommen wird.
--
-- Basis: Migration 0024 (behält atariNumber-nullif und alle Fixes
-- aus 0013/0023). Nur der nests-Block ist neu.
-- ══════════════════════════════════════════════════════════════
...
begin
  ...
  -- atariNumber: nicht-null gewinnt (Fix aus Migration 0024)
  v_out := v_out
    || jsonb_build_object(
  ...
  -- ── lootboxDailyClaimed (max ISO-Datum pro Slot-Key, Fix 0013) ──
  with kv as (
  ...
  -- ── avatarUnlocks (min pro Key, 0 = unbekannt, Fix 0023) ──
  with kv as (
    select key,
      case
  ...
  -- Migration 0025: gameId/gameUrl mit nullif('null'::jsonb) — sonst
  -- friert der JSON-null-Initialwert die Verknüpfung ein und der
  -- Client kann kein Spiel mehr an das Nest hängen.
  with all_nests as (
    select
...
-- ═════════════════════════════════════════════════════════════
-- Cleanup: bestehende JSON-null-Werte in nests[].gameId/gameUrl
-- entfernen. Beim nächsten Sync übernimmt der Client-Wert sauber.
-- ═════════════════════════════════════════════════════════════
...
set value = jsonb_set(
  ...
  (
    select coalesce(
      jsonb_agg(
        case
          when n ? 'gameId'  and jsonb_typeof(n->'gameId')  = 'null'
           and n ? 'gameUrl' and jsonb_typeof(n->'gameUrl') = 'null'
            then (n - 'gameId') - 'gameUrl'
          when n ? 'gameId'  and jsonb_typeof(n->'gameId')  = 'null'
            then n - 'gameId'
          when n ? 'gameUrl' and jsonb_typeof(n->'gameUrl') = 'null'
            then n - 'gameUrl'
          else n
        end
      ),
    ...
    from jsonb_array_elements(uc.value->'nests') as n
  )
)
where uc.key = 'shop_state'
  and jsonb_typeof(uc.value->'nests') = 'array'
  and exists (
    select 1 from jsonb_array_elements(uc.value->'nests') as n
    where (n ? 'gameId'  and jsonb_typeof(n->'gameId')  = 'null')
       or (n ? 'gameUrl' and jsonb_typeof(n->'gameUrl') = 'null')
  );
