-- ══════════════════════════════════════════════════════════════
-- Migration 0030 — Lockmittel (Season-3-Shop-Item)
-- ══════════════════════════════════════════════════════════════
-- Neues Consumable "Lockmittel" (SHOP_ITEMS_P3 in script.js).
-- Wie Glücksklee/Wachstums-Booster: State im shop_state-Blob mit
-- lockmittel (bool, aktiv-Flag) und lockmittelCount (int).
--
-- Ohne diese Migration filtert shop_state_merge den Count/Flag
-- beim nächsten Server-Sync raus (Whitelist-Pattern) und
-- sync_shop_state akzeptiert lockmittelCount nicht im Bereichs-
-- Check (bleibt einfach unvalidiert, aber der Merge-Verlust ist
-- das Problem).
--
-- Basis:
--   shop_state_merge → Migration 0027 (opened_seal_types)
--   sync_shop_state  → Migration 0023 (kristalle_and_seal_merge_fix)
-- ══════════════════════════════════════════════════════════════
...
-- ─────────────────────────────────────────────────────────────
-- sync_shop_state: Bereichscheck für lockmittelCount ergänzen
-- ─────────────────────────────────────────────────────────────
