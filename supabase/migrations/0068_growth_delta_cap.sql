-- ══════════════════════════════════════════════════════════════
-- Migration 0068 — Growth-Delta-Cap auf die Skalengrenze heben
-- ══════════════════════════════════════════════════════════════
-- Wachstum ist die einzige der vier Kennzahlen mit einer harten
-- Obergrenze: GROWTH_S6 = 100 (Stufe 6, „Vollendet"). Ein Delta-Cap
-- UNTER dieser Grenze verbietet deshalb keine unrealistische
-- Steigerung, sondern legitime Ein-Klick-Sprünge:
--
--   • „Backup des Buches" (applyBackupSwap in script.js): setzt
--     growth = GROWTH_THRESHOLDS[stage]; ein vollendetes Monster
--     aus dem Buch heißt growth = 100. Auf einem Slot mit growth < 40
--     ist das ein Delta > 60.
--   • „Stein der Vollendung" / Lootbox-Upgrade: growth 21 → 100,
--     also Delta 79 — immer über dem alten Cap.
--
-- Folge bisher: sync_game_state antwortet 'delta_cap', der Push wird
-- verworfen, der `_pending`-Marker bleibt stehen und jeder Hub-Boot
-- legt einen weiteren cheat_flags-Eintrag an. Lokal sieht alles
-- richtig aus (loadServerState überspringt pending games) — bis
-- localStorage geleert wird (Re-Login via clearLocalGameState) oder
-- ein zweites Gerät lädt. Dann steht das alte Servermonster wieder da.
--
--   growth: 60 → 100  (= volle Skala, ein Sprung von ganz unten nach
--                       ganz oben ist erlaubt)
--
-- Die übrigen Caps bleiben unverändert — points/rounds/coins sind
-- nach oben offen und deshalb die eigentlichen Cheat-Vektoren.
-- Wachstum jenseits von 21 (GROWTH_MAX) bringt ohnehin keine Coins
-- mehr, der Cap schützte dort also nichts.
--
-- Bereits blockierte Pushes heilen von selbst: die `_pending`-Marker
-- stehen noch, pushPendingState() beim nächsten Hub-Boot bringt sie
-- durch.
--
-- Basis: Migration 0031 (letzte Definition der Funktion).
-- ══════════════════════════════════════════════════════════════
...
declare
  ...
  MAX_GROWTH_DELTA  constant int := 100;
