-- ══════════════════════════════════════════════════════════════
-- Migration 0149 — Myth of Wordisland: jede VIERTE Serie zählt doppelt
-- ══════════════════════════════════════════════════════════════
-- Sönke, 15.09.2026:
--   „Ich möchte, dass man bei 4 Streaks in Folge (also bei 12) 2
--    Felder aufdecken kann."
--
-- ── Was bisher passierte ──────────────────────────────────────
-- Seit 0148 ist die Serie ein TAKT: jede dritte sofort richtige
-- Antwort (3, 6, 9 …) bringt genau EINE freie Feldwahl. Das ist
-- richtig, aber flach — die zwölfte richtige Antwort in Folge fühlt
-- sich an wie die dritte, obwohl viermal so viel dahintersteckt.
--
-- Ab hier hat der Takt einen zweiten, größeren Schlag: bei 12, 24,
-- 36 … gibt es ZWEI Wahlen statt einer. Eine Zeile, `case when
-- v_pl.streak % 12 = 0 then 2 else 1 end` — und weil 12 ein
-- Vielfaches von 3 ist, greift sie nur dort, wo ohnehin schon eine
-- Wahl fällig war. Der Takt selbst bleibt unangetastet.
--
-- ⚠️ Die Bedingung davor bleibt Wort für Wort stehen, samt
-- `v_pl.streak > 0`: eine Antwort, die erst über die Auswahl
-- gefunden wurde, setzt die Serie auf null — und `0 % 12` ist
-- ebenfalls 0. Ohne sie schenkte ausgerechnet die Antwort mit Hilfe
-- zwei freie Wahlen (0148 hat denselben Fehler an derselben Stelle
-- im Prüfstand gefunden, nicht im Betrieb).
--
-- ── Warum nicht mehr als zwei ─────────────────────────────────
-- Der Deckel steht seit 0146 bei sechs (wi_players_picks_check), und
-- die Arena schenkt drei auf einmal. Zwei ist damit genau so viel,
-- dass es sich neben der Arena noch unterscheidet: Serie zwölf ist
-- ein guter Lauf, die Arena ist ein eroberter Ort. `least(…, 6)`
-- bleibt unverändert — wer mit fünf offenen Wahlen in die zwölfte
-- Serie läuft, bekommt eine und nicht zwei. Das ist kein Abzug,
-- sondern derselbe Deckel wie überall.
--
-- ── Nur wi_answer, sonst nichts ───────────────────────────────
-- Keine Tabelle, keine Prüfregel, kein neues Feld. Die Funktion steht
-- vollständig da (create or replace, kein DROP — Regel
-- feedback_supabase_no_drop_statements) und ist wörtlich die aus
-- 0148 plus dieser einen Fallunterscheidung.
--
-- ⚠️ Fehlt diese Migration, ist das Spiel nicht kaputt, sondern
-- sparsamer: bei Serie 12 gibt es eine Wahl statt zweier, und der
-- Wahl-Kasten im Gerät schreibt dann ehrlich „Nimm 1 Feld gezielt
-- ein!" — er liest die Zahl vom Server und rechnet sie nicht nach
...
begin
  ...
  if v_ok then
    ...
    if not v_solo then
      ...
      -- Antworten dazwischen wieder ein zufälliges Nachbarfeld (0148).
      --
      -- Und JEDE VIERTE dieser Serien zählt doppelt: bei 12, 24, 36 …
      -- sind es ZWEI Wahlen (0149). Die Zahl steht hier und nur hier;
      -- das Gerät liest sie aus der Antwort und rechnet sie nicht nach.
      --
      -- ⚠️ `streak > 0` gehört zwingend dazu: eine Antwort, die erst
      -- über die Auswahl gefunden wurde, SETZT die Serie auf null
      -- (ein paar Zeilen weiter oben) — und 0 % 3 (wie 0 % 12) ist
      ...
      if v_pl.streak > 0 and v_pl.streak % 3 = 0 then
        update wi_players
           set picks = least(picks + case when v_pl.streak % 12 = 0 then 2 else 1 end, 6)
         where participant_id = v_p.id;
      else
...
comment on function wi_answer(text, text) is
  ...
  'eine freie Feldwahl, jede ZWÖLFTE deren zwei (0149), alle anderen ein zufälliges '
  'Nachbarfeld.';
...
comment on column wi_players.picks is
  'Freie Feldwahl. Wächst mit jeder DRITTEN sofort richtigen Antwort in Folge um eins, mit '
  'jeder ZWÖLFTEN um zwei (0149), und fällt mit der Serie auf 0. Die Arena schenkt drei auf '
  'einmal — Deckel sechs seit 0146.';
