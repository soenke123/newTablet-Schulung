-- ══════════════════════════════════════════════════════════════
-- Migration 0027 — Siegel-Eier nach Öffnung endgültig entfernen
-- ══════════════════════════════════════════════════════════════
-- Bug: triggerSealEggOpening (script.js) entfernt das Siegel-Ei
-- nur clientseitig aus sd.sealedEggs. Der Server-Merge macht
-- Union pro Type (Migration 0023/0026) — das Server-Ei mit
-- allen 4 seals[i].solved=true kommt beim nächsten Sync zurück,
-- Card erscheint erneut mit "✨ Ei öffnen!" → User kann das
-- Legi-Monster beliebig oft erzeugen. Legis sollen aber unique
-- sein.
--
-- Fix: Neuer Client-Blob-Feld `openedSealTypes: text[]`, das
-- beim Öffnen die geöffneten Types trackt. Analog zu `purchased`
-- monoton wachsend, Union-Merge. Der sealedEggs-Merge filtert
-- alle Types raus, die bereits in openedSealTypes stehen.
--
-- Kein Backfill: User mit bestehenden Duplikaten haben ihre
-- Bonus-Kreaturen behalten dürfen. Ab jetzt gilt aber: einmal
-- öffnen, dann für immer weg.
--
-- Basis: Migration 0026 (nest_reset_authoritative).
-- ══════════════════════════════════════════════════════════════
...
begin
  ...
  -- Client trägt beim Öffnen den Type ein (siehe triggerSealEggOpening).
  -- Der Server merged Union — einmal geöffnet = für immer geöffnet.
  ...
  -- ── sealedEggs[] ─────────────────────────────────────────
  -- Alle Eier aus (Server ∪ Client) pro Type gruppieren, seals-Flags
  -- per OR mergen. Wenn Type in openedSealTypes → droppen.
  with all_pool as (
