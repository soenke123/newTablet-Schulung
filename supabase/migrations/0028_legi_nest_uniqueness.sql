-- ══════════════════════════════════════════════════════════════
-- Migration 0028 — Legi-Kreaturen im Nest-Merge deduplizieren
-- ══════════════════════════════════════════════════════════════
-- Härtung 2 zur Legi-Uniqueness. Migration 0027 verhindert dass
-- ein Siegel-Ei nach Öffnung wieder auftaucht. Ergänzt hier: falls
-- durch Two-Tab-Race, alten Cheat-State oder direkte REST-Injektion
-- doch mehrere Nests mit derselben Legi-Kreatur entstanden sind,
-- lässt der Merge nur einen davon durch — den mit dem höchsten
-- Fortschritt (growth, roundsPlayed, points, coins).
--
-- Legi-Set: chinDrache (himmel), schnabeltier (suempfe). Robot und
-- Pfau haben eigene Uniqueness-Pfade (atariSolved, pfauEggGranted)
-- und sind hier nicht relevant.
--
-- Nicht-Legi-Nests bleiben unangetastet — zwei "huhn"-Kreaturen in
-- verschiedenen Slots sind legitim.
--
-- Basis: Migration 0027 (opened_seal_types).
-- ══════════════════════════════════════════════════════════════
...
begin
  ...
  -- ── nests[] ──────────────────────────────────────────────
  -- Standard-Merge wie in Migration 0026, plus:
  -- Legi-Dedup am Ende — bei Duplikaten (mehrere Nests mit
  -- hatched.creature ∈ Legi-Set) bleibt der Nest mit maximalem
  -- (growth, roundsPlayed, points, coins) übrig.
  with all_nests as (
    select
  ...
  -- Legi-Dedup: pro Legi-Kreatur nur den besten Nest behalten
  ranked as (
    select
      n,
      n->'hatched'->>'creature' as creat,
      row_number() over (
        partition by n->'hatched'->>'creature'
        order by
          coalesce((n->'hatched'->>'growth')::int, 0)       desc,
          coalesce((n->'hatched'->>'roundsPlayed')::int, 0) desc,
          coalesce((n->'hatched'->>'points')::int, 0)       desc,
          coalesce((n->'hatched'->>'coins')::int, 0)        desc,
          n->>'nestId'                                       asc
      ) as rn
    from merged_nests
  ),
  filtered as (
    select n from ranked
    where creat is null
       or creat not in ('chinDrache','schnabeltier')
       or rn = 1
  )
  select coalesce(jsonb_agg(n), '[]'::jsonb) into v_nests from filtered;
