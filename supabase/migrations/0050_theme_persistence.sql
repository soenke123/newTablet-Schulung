...
-- ══════════════════════════════════════════════════════════════
-- Bug: _getUnlockedThemes (script.js §13) prüft LIVE, ob eine
-- Kreatur aktuell im Besitz UND gemaxed ist. Wird das Monster
-- freigelassen (Nest-Reset, ins Buch), verschwindet das dazu-
-- gehörige Theme sofort — obwohl der User es sich verdient hat.
--
-- Fix: shop_state bekommt zwei neue Keys:
--   • unlockedThemes: text[]  — Union-Merge, monoton wachsend.
--     Einmal ein Theme freigeschaltet (= Monster erreichte max
--     growth), bleibt es für immer. Analog zu openedSealTypes
--     (Migration 0027) / purchased[].
--   • activeTheme: text       — client-autoritativ (letzter
--     Schreiber gewinnt). Ersetzt localStorage.lernwelt_theme_pref
--     als Source-of-Truth und synct Theme-Wahl cross-device.
--
-- Kein Backfill: Bestandsuser bekommen ihre alten Themes über
-- den Client-Live-Check + localStorage-Fallback beim ersten
-- Login zurückgemerged.
--
-- Basis: Migration 0044 (komplette shop_state_merge übernommen,
-- feedback_shop_state_merge_regressions folgend).
-- ══════════════════════════════════════════════════════════════
...
begin
  ...
  -- NEU Migration 0050: Themes bleiben einmal freigeschaltet
  -- für immer freigeschaltet, egal ob Trigger-Kreatur noch
  -- im Besitz ist.
  ...
  -- NEU Migration 0050: Cross-Device-Sync der Theme-Wahl.
  v_out := v_out
    || jsonb_build_object(
