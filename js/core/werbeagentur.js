...
(function (RT) {
  ...
  RT.bus.on('month:advance', function () {
    ...
    if (totalRepDelta !== 0) RT.state.dispatch('ADD_REPUTATION', { delta: totalRepDelta });
  });
  ...
  function render() {
    ...
    var previewHtml = '<div style="margin-bottom:14px;">'
      + '<div style="font-size:0.72em;text-transform:uppercase;letter-spacing:.05em;opacity:0.5;margin-bottom:6px;">Vorschau (ausstehende Änderungen)</div>'
      + '<div style="display:flex;gap:8px;flex-wrap:wrap;">'
      + '<div style="' + cardStyle + 'background:rgba(231,76,60,0.10);">'
      +   '<span style="font-size:0.7em;text-transform:uppercase;letter-spacing:.04em;opacity:0.6;">Ruf/Monat</span>'
      +   '<strong data-preview="rep" style="font-size:1.15em;">' + fmtRep(preview.repDelta) + '</strong>'
      + '</div>'
      ...
      +   '<strong data-preview="eur" style="font-size:1.15em;">' + fmtEurUser(preview.revenue) + '</strong>'
      + '</div>'
      ...
      +   '<strong data-preview="total" style="font-size:1.15em;">' + fmtEurTotal(preview.revenue * users) + '</strong>'
      + '</div>'
      + '</div>'
      + '</div>';
  ...
  function updatePreview() {
    ...
    if (repEl) repEl.textContent = fmtRep(preview.repDelta);
    if (eurEl) eurEl.textContent = fmtEurUser(preview.revenue);
    if (totEl) totEl.textContent = fmtEurTotal(preview.revenue * users);
  }
