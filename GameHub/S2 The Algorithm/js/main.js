  ...
  if (!gd.creature) {
    ...
    if (ALG_GAME_ID.startsWith('nest_')) {
      ...
      if (nest && !nest.gameUrl) {
        nest.gameUrl = 'S2 The Algorithm/index.html';
        saveShopData(sd);
      }
    }
  } else {
...
function tick() {
  ...
  if (syn.tripleGruen) {
    ...
    gameState.sozialdrang = Math.min(100, gameState.sozialdrang + 1);
  }
  ...
  gameState.reizschwelle = Math.max(0, Math.min(100, gameState.reizschwelle - 0.5));
